...
public class TestFixtureInitializer{
    ...
    public static FileSystem createFS(){
        ...
        File A = new File(main, "A.java", 20, currentTime);
        main.appendChild(A);
        File B = new File(main, "B.java", 15, currentTime);
        main.appendChild(B);
        File ATest = new File(test, "ATest.java", 13, LocalDateTime.now());
        test.appendChild(ATest);
        File BTest = new File(test, "BTest.java", 12, LocalDateTime.now());
        test.appendChild(BTest);
        Link rm = new Link(test, "rm.md", 0, LocalDateTime.now(), readme);
        test.appendChild(rm);
